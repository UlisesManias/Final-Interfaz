...
public class ControladorCreacion {
    ...
    private void borrarHeroe() {
        ...
        int confirmacion = JOptionPane.showConfirmDialog(vista,
                "¿Está seguro de borrar el héroe '" + heroeSeleccionado.getNombre() + "' ("
                        + heroeSeleccionado.getApodo() + ")?",
                "Confirmar",
                JOptionPane.YES_NO_OPTION);

        if (confirmacion == JOptionPane.YES_OPTION) {
            GestorPersistencia.eliminarPersonaje(heroeSeleccionado.getApodo());
            heroeSeleccionado = null;
            cargarPersonajesEnCombos();
            // Limpiar campos luego de borrar
            vista.getSpnVidaH2().setValue(0);
            vista.getSpnFuerzaH2().setValue(0);
            vista.getSpnDefensaH2().setValue(0);

            JOptionPane.showMessageDialog(vista, "Héroe borrado correctamente de la base de datos");
        }
    }
    ...
    private void borrarVillano() {
        ...
        int confirmacion = JOptionPane.showConfirmDialog(vista,
                "¿Está seguro de borrar el villano '" + villanoSeleccionado.getNombre() + "' ("
                        + villanoSeleccionado.getApodo() + ")?",
                "Confirmar",
                JOptionPane.YES_NO_OPTION);

        if (confirmacion == JOptionPane.YES_OPTION) {
            GestorPersistencia.eliminarPersonaje(villanoSeleccionado.getApodo());
            villanoSeleccionado = null;
            cargarPersonajesEnCombos();
            // Limpiar campos luego de borrar
            vista.getSpnVidaV2().setValue(0);
            vista.getSpnFuerzaV2().setValue(0);
            vista.getSpnDefensaV2().setValue(0);

            JOptionPane.showMessageDialog(vista, "Villano borrado correctamente de la base de datos");
        }
    }
    ...
    private void siguiente() {
        ...
        // Transferir estadísticas históricas
        heroeCopia.setVictorias(heroeSeleccionado.getVictorias());
        heroeCopia.setDerrotas(heroeSeleccionado.getDerrotas());
        heroeCopia.setArmasInvocadas(heroeSeleccionado.getArmasInvocadas());
        heroeCopia.setAtaquesSupremosUsados(heroeSeleccionado.getAtaquesSupremosUsados());

        villanoCopia.setVictorias(villanoSeleccionado.getVictorias());
        villanoCopia.setDerrotas(villanoSeleccionado.getDerrotas());
        villanoCopia.setArmasInvocadas(villanoSeleccionado.getArmasInvocadas());
        villanoCopia.setAtaquesSupremosUsados(villanoSeleccionado.getAtaquesSupremosUsados());
